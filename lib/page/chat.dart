...
Future<void> sendMessageWithImage() async {
  try {
    ...
    if (_image != null) {
      imageUrl = await _uploadImageToStorage(_image!); // Upload image only if selected
    }
...
Widget buildMessage(Map<String, dynamic> message) {
  ...
  final imageUrl = message['imageUrl']; // Image URL fetched from Firebase Storage
...
Future<String> _uploadImageToStorage(File imageFile) async {
  try {
    String fileName = DateTime.now().millisecondsSinceEpoch.toString(); // Unique filename based on timestamp
